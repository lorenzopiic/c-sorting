 ...
 * che tipo di oggetto allocare, se array, matrice o matrice di puntatori */
    ...
    if (status == INPUT_ERROR ||
        status == READ_ERROR || status == MALLOC_ERROR) {
        fprintf(stderr, "%s\n", warnmsg[status]);
        return 1;
    }
 ...
 * operare correttamente la divisione, il cui modulo restituisce correttamente il wraper da chiamare. */
    ...
    if (algID < 0 || algID > 2) {
        fprintf(stderr,
                "ERRORE INTERNO: Indice algID errato (%d). Status: %d\n",
                algID, status);
        return 1;
    }
    doSort[algID] (mydata);
 ...
 * Il secondo switch annidato gestisce in maniera del tutto analoga typeID */
    ...
    case 1:                    // INSERTION
        ...
        case 1:
            ...
            break;
        case 2:
            return INSERTION_POINTERS_MATRIX;
            break;
        }
        break;
    ...
    case 2:                    // SELECTION
        ...
        case 1:
            ...
            break;
        case 2:
            return SELECTION_POINTERS_MATRIX;
            break;
        }
        break;
    }                           // END SWITCH //
...
double *randomizeArray(void)
{
    ...
    double max = ARRAYSIZE;
    double min = ARRAYSIZE - (ARRAYSIZE / 2);
...
{
    if (dt.exitStatus == MALLOC_ERROR)
        return;                 /* Nulla da liberare */
...
void bubbleArray(double *arr)
{
    ...
    for (unsigned p = 0; p < ARRAYSIZE - 1; p++) {
        /* sottraiamo il numero di passate */
        for (size_t i = 0; i < ARRAYSIZE - 1 - p; i++) {
            if (arr[i] > arr[i + 1]) {
                double temp = arr[i];
                arr[i] = arr[i + 1];
                arr[i + 1] = temp;
...
/*    
    // Calcoliamo il numero totale di celle come se fosse un array unico
    unsigned totalElements = ROWS * COLS; 

    // Ciclo esterno (Passate): identico al Bubble Sort classico
    for(unsigned p = 0; p < totalElements - 1; p++){
        
        // Ciclo interno: scorre l'indice "lineare" k
        // Nota l'ottimizzazione "- p" per non ricontrollare la fine ordinata
        for(unsigned k = 0; k < totalElements - 1 - p; k++){
            
            // --- CONVERSIONE COORDINATE ---
            // Coordinate dell'elemento CORRENTE (k)
            unsigned r1 = k / COLS;
            unsigned c1 = k % COLS;

            // Coordinate dell'elemento SUCCESSIVO (k+1)
            unsigned r2 = (k + 1) / COLS;
            unsigned c2 = (k + 1) % COLS;

            // --- CONFRONTO E SCAMBIO ---
            if(mat[r1][c1] > mat[r2][c2]){
                double temp = mat[r1][c1];
                mat[r1][c1] = mat[r2][c2];
                mat[r2][c2] = temp;
            }
        }
...
/* PREMESSA: Questa versione con [0] è formalmente meno efficiente perché implica un'operazione di aritmetica dei puntatori 
 * (calcolare indirizzo + offset 0) invece della dereferenziazione diretta. Inoltre, è semanticamente improprio trattare
 * un singolo valore scalare come se fosse un array, rendendo il codice meno chiaro rispetto all'uso dell'asterisco *  */
...
/*
    ...
    for(unsigned p=0; p < totalElements - 1; p++){
        for(unsigned k=0; k < totalElements - 1 - p; k++){
            ...
            // Calcolo coordinate da indice lineare k
            unsigned r1 = k / COLS;
            unsigned c1 = k % COLS;
            ...
            if(*pmat[r1][c1] > *pmat[r2][c2]){
                double *temp = pmat[r1][c1];
                pmat[r1][c1] = pmat[r2][c2];
                pmat[r2][c2] = temp;
            }
        }
    }
